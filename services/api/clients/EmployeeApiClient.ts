...
export class EmployeeApiClient extends BaseApiClient {
  ...
  async getEmployeeById(id: number): Promise<Employee> {
    if (!id || id <= 0) {
      throw createValidationError(ERROR_MESSAGES.INVALID_EMPLOYEE_ID, {
  ...
  async addEmployee(params: EmployeeUpdateParams): Promise<number> {
    // Validate required fields
    if (!params.name || params.name.trim() === "") {
      throw createValidationError(ERROR_MESSAGES.EMPLOYEE_NAME_REQUIRED, {
        operation: API_OPERATIONS.ADD_EMPLOYEE,
      });
    }

    if (!params.nip || params.nip.trim() === "") {
      throw createValidationError("Employee NIP is required", {
        operation: "addEmployee",
      });
    }

    if (!params.position || params.position.trim() === "") {
      throw createValidationError("Employee position is required", {
        operation: "addEmployee",
      });
    }

    const sanitizedParams = this.sanitizeEmployeeParams(params);

    const payload = this.mapEmployeeToApi({
      name: sanitizedParams.name,
      nip: sanitizedParams.nip,
      gol: sanitizedParams.gol,
      pangkat: sanitizedParams.pangkat,
      position: sanitizedParams.position,
      subPosition: sanitizedParams.subPosition,
      organizationalLevel: sanitizedParams.organizationalLevel,
    });
  ...
  async updateEmployee(
    ...
    // Validate required fields
    if (!params.name || params.name.trim() === "") {
      throw createValidationError("Employee name is required", {
        operation: "updateEmployee",
      });
    }
  ...
  async importEmployeesFromCSV(
    ...
    if (employees.length === 0) {
      throw createValidationError("Cannot import empty employee list", {
        operation: "importEmployeesFromCSV",
      });
    }
  ...
  async getEmployeeOrgLevelMapping(
    signal?: AbortSignal,
  ...
  async getEmployeeSuggestions(name: string): Promise<EmployeeSuggestion[]> {
    if (!name || name.trim() === "") {
      throw createValidationError("Employee name is required for suggestions", {
  ...
  async resolveEmployees(
    mappings: Record<string, string>,
    sessionId: string,
  ): Promise<void> {
    if (!mappings || Object.keys(mappings).length === 0) {
      throw createValidationError("Employee mappings are required", {
  ...
  async updateEmployeeSummary(
    ...
    await this.put<void>(
      `/employees/${encodeURIComponent(employeeName)}/summary`,
      {
        sessionId,
        summary,
      },
  ...
  private sanitizeEmployeeParams(
    params: EmployeeUpdateParams,
    ...
    return {
      ...
      nip: params.nip?.trim(),
      gol: params.gol?.trim(),
      pangkat: params.pangkat?.trim(),
      position: params.position?.trim(),
      subPosition: params.subPosition?.trim(),
      organizationalLevel: params.organizationalLevel?.trim(),
    };
  }
  ...
  private mapEmployeeToApi(
    ...
    const organizationalLevel =
      employee.organizationalLevel ?? employee.organizational_level;
