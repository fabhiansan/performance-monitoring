...
export class SessionApiClient extends BaseApiClient {
  ...
  async saveEmployeeData(employees: Employee[], sessionName?: string): Promise<string> {
    ...
    const validatedEmployees = employees.map((emp, index) => {
      ...
      if (!Array.isArray(performance)) {
        logger.warn('Invalid performance data for employee, converting to array', { employeeName: emp.name });
      }
    ...
    const result = await this.post<SessionSaveResponse>('/employee-data', {
    ...
    logger.api(`Saved employees to session`, { employeeCount: validatedEmployees.length, sessionId: result.sessionId });
  ...
  async getEmployeeDataBySession(sessionId: string, signal?: AbortSignal): Promise<EmployeeWithSession[]> {
    ...
    if (!result || !Array.isArray(result.employees)) {
      throw createServerError(
        ERROR_MESSAGES.INVALID_RESPONSE_FORMAT,
    ...
    const employees = result.employees.map((emp: EmployeeWithSession) => {
      ...
      if (emp.performance && !Array.isArray(emp.performance)) {
        logger.warn('Invalid performance data for employee, converting to empty array', { employeeName: emp.name });
        emp.performance = [];
      }
      ...
      return {
        ...
        ...(emp.uploadTimestamp ? { uploadTimestamp: emp.uploadTimestamp } : {}),
    ...
    if (result.metadata) {
      logger.api('Session data metadata', { metadata: result.metadata });
      
      if (result.metadata.employeesWithoutPerformanceData > 0) {
        logger.warn('Employees loaded without performance data', { count: result.metadata.employeesWithoutPerformanceData });
      }
    }
  ...
  async setCurrentSession(sessionId: string): Promise<void> {
    ...
    await this.post<void>('/current-session', {
      sessionId
    }, {
      operation: 'setCurrentSession'
    });
