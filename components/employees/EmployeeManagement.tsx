...
const EmployeeManagement: React.FC<EmployeeManagementProps> = ({ employees: propEmployees = [], onEmployeeUpdate }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [showImport, setShowImport] = useState(false);
  const [showAddForm, setShowAddForm] = useState(false);
  const [editingEmployee, setEditingEmployee] = useState<(Employee & { id: number }) | null>(null);
  ...
  // Use employees from props, filtering out any without IDs (for employee management operations)
  const employees = propEmployees.filter((emp): emp is Employee & { id: number } => 
    emp.id !== undefined && emp.id !== null
  );

  const handleEditEmployee = (employee: Employee & { id: number }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await api.bulkDeleteEmployees(employeeIds);
      setSelectedEmployees(new Set());
  ...
  return (
    <div className="space-y-6">
      ...
      {(showAddForm || editingEmployee) && (
        <AddEmployeeForm
          employee={editingEmployee as Employee | undefined}
