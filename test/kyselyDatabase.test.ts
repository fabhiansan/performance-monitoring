...
describe("KyselyDatabaseService", () => {
  ...
  beforeEach(async () => {
    ...
    for (const session of sessions) {
      await db.deleteUploadSession(session.period);
    }
  ...
  describe("Employee Operations", () => {
    ...
    it("should summarize organizational level counts", async () => {
      await db.addEmployee({
        name: "Org Level Manager",
        nip: "33333333333333333",
        gol: "IV/a",
        pangkat: "Pembina",
        position: "Manager",
        sub_position: "Bidang A",
        organizational_level: "Eselon II",
      });

      await db.addEmployee({
        name: "Org Level Staff",
      ...
      const counts = await db.getOrgLevelCounts();
      expect(counts["Eselon II"]).toBeGreaterThanOrEqual(1);
      expect(counts[STAFF_OTHER]).toBeGreaterThanOrEqual(1);
    });

    it("should bulk delete employees by identifiers", async () => {
      const firstId = await db.addEmployee({
        name: "Bulk Delete 1",
        nip: "55555555555555555",
        gol: "III/b",
        pangkat: "Penata",
        position: "Staff",
        sub_position: "Unit I",
      ...
      const secondId = await db.addEmployee({
        name: "Bulk Delete 2",
        nip: "66666666666666666",
        gol: "III/b",
        pangkat: "Penata",
        position: "Staff",
        sub_position: "Unit II",
        organizational_level: STAFF_OTHER,
      });

      await db.bulkDeleteEmployees([firstId, secondId]);
      const employees = await db.getAllEmployees();
      expect(
        employees.find((employee) => employee.name === "Bulk Delete 1"),
      ).toBeUndefined();
      expect(
        employees.find((employee) => employee.name === "Bulk Delete 2"),
      ).toBeUndefined();
    });
  });
  ...
  describe("Session Operations", () => {
    ...
    it("should get all upload sessions", async () => {
      ...
      expect(session).toMatchObject({
        ...
        latest_upload: expect.any(String),
      ...
      const employees: Employee[] = [
        {
          ...
          gol: "IV/b",
          organizational_level: STAFF_OTHER,
          performance: [{ name: "Leadership", score: 88 }],
        },
      ];

      const sessionId = await db.saveEmployeeData(
        employees,
        "Data Retrieval Test",
      );
    ...
    it("should delete upload session", async () => {
      const employees: Employee[] = [
        {
          ...
          gol: "I/a",
          organizational_level: STAFF_OTHER,
          performance: [{ name: "Disiplin", score: 80 }],
        },
      ];
      ...
      expect(sessions.some((s) => s.period === sessionId)).toBe(true);
      ...
      expect(sessions.some((s) => s.period === sessionId)).toBe(false);
    });
  });
      ...
      const employees: Employee[] = [
        {
          ...
          gol: "II/d",
          organizational_level: STAFF_OTHER,
          performance: [{ name: "Problem Solving", score: 92 }],
        },
      ];
  ...
  describe("Leadership Score Operations", () => {
    ...
    beforeEach(async () => {
      ...
      datasetId = await db.saveCurrentDataset(
        employees,
        "Leadership Test Dataset",
      );
    });
    ...
    it("should return empty array for dataset with no leadership scores", async () => {
      const scores = await db.getAllManualLeadershipScores(
        "nonexistent-dataset",
      );
      expect(Array.isArray(scores)).toBe(true);
  ...
  describe("Data Integrity", () => {
    it("should handle missing required fields gracefully", async () => {
      ...
      await expect(db.addEmployee(incompleteEmployee)).resolves.toBeTypeOf(
        "number",
      );
    });
    ...
    it("should maintain referential integrity when deleting sessions", async () => {
      ...
      // Verify data exists
      const employeesForSession = await db.getEmployeeDataBySession(sessionId);
      expect(employeesForSession.length).toBe(1);

      // Delete session (should cascade delete related data)
      await db.deleteUploadSession(sessionId);

      // Verify all related data is deleted
      const deletedSessionEmployees =
        await db.getEmployeeDataBySession(sessionId);
