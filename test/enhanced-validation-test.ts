...
import { validatePerformanceData, VALIDATION_LIMITS } from '../services/validationService';
import { Employee } from '../types';

// Constants for repeated strings
const TEST_EMPLOYEE_NAME = 'Test Employee';
const TEST_NIP = '123456789';
const PENATA_MUDA = PENATA_MUDA;
const KUALITAS_KINERJA = KUALITAS_KINERJA;
...
const testArraySizeLimits = () => {
  ...
  const employeeWithTooManyCompetencies: Employee = {
    id: 1,
...
const testCircularReferences = () => {
  ...
  const employeesWithDuplicateIds: Employee[] = [
    {
      id: 1,
      name: 'John Doe',
      nip: TEST_NIP,
      ...
      organizational_level: 'Staff ASN',
      performance: [
        { name: KUALITAS_KINERJA, score: 85 },
        { name: KUALITAS_KINERJA, score: 90 } // Duplicate competency
      ]
    },
    {
...
const testCompetencyNameSanitization = () => {
  ...
  const employeeWithBadNames: Employee = {
    id: 1,
    name: TEST_EMPLOYEE_NAME,
    nip: TEST_NIP,
    gol: 'III/a',
    ...
    performance: [
      ...
      { name: null as unknown as string, score: 85 }, // Null name
...
const testDuplicateCompetencyMerging = () => {
  ...
  const employeeWithDuplicates: Employee = {
    id: 1,
    name: TEST_EMPLOYEE_NAME,
    nip: TEST_NIP,
    gol: 'III/a',
  ...
  console.log(`   📊 Original competency count: ${employeeWithDuplicates.performance?.length ?? 0}`);
  ...
  console.log(`   📊 Final competency count: ${employeeWithDuplicates.performance?.length ?? 0}`);
};
...
const testCombinedEdgeCases = () => {
  ...
  const complexTestData: Employee[] = [
    {
      id: 1,
      ...
      performance: [
        { name: '  kualitas@kinerja  ', score: 85 }, // Needs sanitization
        { name: KUALITAS_KINERJA, score: 90 }, // Duplicate after sanitization
  ...
  const secondEmployee = complexTestData[1];
  if (secondEmployee?.performance) {
    for (let i = 0; i < 30; i++) {
      secondEmployee.performance.push({
        name: `Competency ${i + 1}`,
        score: 80 + (i % 20)
      });
    }
