 ...
 */
const LoadingSpinner: React.FC<{ size: ButtonProps["size"] }> = ({
  size = "md",
}) => {
  const sizeMap = {
    xs: "w-3 h-3",
...
export const Button = forwardRef<React.ElementRef<"button">, ButtonProps>(
  (
    ...
    const handleKeyDown = (
      event: React.KeyboardEvent<React.ElementRef<"button">>,
    ) => {
      if (onKeyDown) {
        onKeyDown(event);
      }
      ...
      const keyboardHandler = keyboard.createButtonHandler(() => {
        if (onClick && !disabled && !loading) {
          onClick(
            event as unknown as React.MouseEvent<React.ElementRef<"button">>,
          );
        }
      });
    ...
    // Handle click events
    const handleClick = (
      event: React.MouseEvent<React.ElementRef<"button">>,
    ) => {
