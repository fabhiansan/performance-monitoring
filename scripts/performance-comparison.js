...
class PerformanceComparison {
  ...
  testCompetencyFiltering() {
    ...
    try {
      ...
      console.log('⚠️ Denormalized query failed (table may not exist):', error.message);
    }
    ...
    try {
      ...
      console.log('⚠️ Normalized query failed (table may not exist):', error.message);
    }
  }
  ...
  testCompetencyAverages() {
    ...
    try {
      ...
      console.log('⚠️ Denormalized query failed:', error.message);
    }
    ...
    try {
      ...
      console.log('⚠️ Normalized query failed:', error.message);
    }
  }
    ...
    try {
      ...
      for (const table of tables) {
        try {
          ...
          console.log(`   ${table.name}: ${count} rows, ${size?.size || 'N/A'} bytes`);
        } catch (error) {
          console.log(`   ${table.name}: Error getting stats - ${error.message}`);
        }
      }
  ...
  generateReport() {
    ...
    for (const [queryType, results] of Object.entries(grouped)) {
      ...
      const denormalized = results.find(r => r.name.includes('Denormalized'));
      const normalized = results.find(r => r.name.includes('Normalized'));
    ...
    if (denormalizedResults.length > 0 && normalizedResults.length > 0) {
      const avgDenormalized = denormalizedResults.reduce((sum, r) => sum + r.avgTimeMs, 0) / denormalizedResults.length;
      const avgNormalized = normalizedResults.reduce((sum, r) => sum + r.avgTimeMs, 0) / normalizedResults.length;
