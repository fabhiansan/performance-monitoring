...
export const SIMPLIFIED_ORGANIZATIONAL_LEVEL_STYLES = {
  ESELON: {
    label: "Eselon",
    color: colors.blue[600],
    bgColor: colors.blue[50],
...
export const ANIMATION = {
  ...
  EASING_ELASTIC: "cubic-bezier(0.68, -0.6, 0.32, 1.6)",
} as const;

// Micro-animation presets
export const MICRO_ANIMATIONS = {
  SCALE_UP: "transition-transform duration-200 ease-out hover:scale-105",
  SCALE_DOWN: "transition-transform duration-200 ease-out active:scale-95",
  LIFT: "transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-lg",
  GLOW: "transition-shadow duration-300 ease-out hover:shadow-xl hover:shadow-blue-500/20",
  ROTATE: "transition-transform duration-300 ease-out hover:rotate-3",
...
export const getLegacyPerformanceLevel = (score: number) => {
  if (score >= LEGACY_PERFORMANCE_LEVELS.SANGAT_BAIK.min)
    return LEGACY_PERFORMANCE_LEVELS.SANGAT_BAIK;
  if (score >= LEGACY_PERFORMANCE_LEVELS.BAIK.min)
    return LEGACY_PERFORMANCE_LEVELS.BAIK;
  if (score >= LEGACY_PERFORMANCE_LEVELS.KURANG_BAIK.min)
    return LEGACY_PERFORMANCE_LEVELS.KURANG_BAIK;
  return LEGACY_PERFORMANCE_LEVELS.PERLU_PERBAIKAN;
};

export const getOrganizationalLevelStyle = (level: string) => {
  const simplified = simplifyOrganizationalLevel(level);
  return simplified === "Eselon"
    ? SIMPLIFIED_ORGANIZATIONAL_LEVEL_STYLES.ESELON
    : SIMPLIFIED_ORGANIZATIONAL_LEVEL_STYLES.STAFF;
};

// Export types for TypeScript
export type PerformanceLevel =
  (typeof PERFORMANCE_LEVELS)[keyof typeof PERFORMANCE_LEVELS];
export type LegacyPerformanceLevel =
  (typeof LEGACY_PERFORMANCE_LEVELS)[keyof typeof LEGACY_PERFORMANCE_LEVELS];
export type OrganizationalLevel =
  (typeof ORGANIZATIONAL_LEVELS)[keyof typeof ORGANIZATIONAL_LEVELS];
