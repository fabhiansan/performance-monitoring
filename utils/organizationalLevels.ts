 ...
 */
export type OrganizationalCategory = 'Eselon II' | 'Eselon III' | 'Eselon IV' | 'Staff' | 'Other';
...
const normalizeOrganizationalLevel = (level: string): string => {
  ...
  const abbreviationMap: Record<string, string> = {
    ...
    'staff non asn perlindsos': 'staff non asn bidang perlindungan dan jaminan sosial',
 ...
 */
export const parseGolongan = (golongan: string | undefined | null): ParsedGolongan | null => {
 ...
 */
export const isValidGolongan = (golongan: string | undefined | null): boolean => {
 ...
 */
export const formatGolongan = (golongan: string | undefined | null): string | null => {
 ...
 */
export const getGolonganDisplayName = (level: GolonganLevel, grade: GolonganGrade): string => {
 ...
 */
export const inferEselonFromGolongan = (golongan: string | undefined | null): OrganizationalCategory => {
  const parsed = parseGolongan(golongan);
 ...
 */
export const isValidOrganizationalLevel = (level: string): level is OrganizationalLevel => {
  ...
  return ORGANIZATIONAL_LEVELS.some(validLevel => 
...
const validateDataConsistency = (
  ...
  if (levelDifference >= 2) {
    ...
    const severity: 'low' | 'medium' | 'high' = 
      levelDifference >= 3 ? 'high' : 
      levelDifference === 2 ? 'medium' : 'low';
 ...
 */
const logDataInconsistencyWarning = (warning: DataInconsistencyWarning): void => {
  const logLevel = warning.severity === 'high' ? 'error' : 
                   warning.severity === 'medium' ? 'warn' : 'info';
  
  console[logLevel](`[Data Inconsistency - ${warning.severity.toUpperCase()}] ${warning.message}`);
};
...
export const validateOrganizationalDataConsistency = (
  ...
  return validateDataConsistency(golonganInference, positionInference, golongan);
};
...
export const isEselonLevel = (level: string | undefined | null): boolean => {
  ...
  return category === 'Eselon II' || category === 'Eselon III' || category === 'Eselon IV';
};
...
export const determineOrganizationalLevelFromPosition = (
  ...
  for (const pattern of eselonIIPatterns) {
  ...
  for (const pattern of eselonIVPatterns) {
    if (pattern.test(fullContext)) {
      return 'Eselon IV';
    }
  }
  
  // Check if it's a staff position
  const staffPatterns = [
    /\bstaf+\b/,
    /\bstaff\b/,
    /\banalis\b/,
  ...
  if (normalizedSubPosition && (
    normalizedSubPosition.includes('bidang') ||
    normalizedSubPosition.includes('sekretariat') ||
    normalizedSubPosition.includes('bagian') ||
    normalizedSubPosition.includes('seksi')
  )) {
    return 'Staff';
  }
 ...
 */
export const getPositionType = (employee: { organizational_level?: string; position?: string }): PositionType => {
  ...
  if (category === 'Eselon II' || category === 'Eselon III' || category === 'Eselon IV') {
    return 'eselon';
  }
 ...
 * @returns 'eselon' for Eselon levels, 'staff' for Staff levels
 */
export const getPositionTypeByLevel = (level: string | undefined | null): PositionType => {
 ...
 */
export const getSpecificEselonLevel = (level: string | undefined | null): 'Eselon II' | 'Eselon III' | 'Eselon IV' | 'Staff' => {
  const category = categorizeOrganizationalLevel(level);
  if (category === 'Eselon II' || category === 'Eselon III' || category === 'Eselon IV') {
    return category;
  }
 ...
 */
export const getUniqueOrganizationalLevels = (employees: Array<{ organizational_level?: string }>): string[] => {
 ...
 */
export const groupEmployeesByOrganizationalLevel = <T extends { organizational_level?: string }>(
  employees: T[]
): Record<string, T[]> => {
 ...
 */
export const countEmployeesByOrganizationalLevel = (employees: Array<{ organizational_level?: string }>) => {
...
export const validateOrganizationalLevel = (level: string) => {
  ...
  const suggestions = !isValid ? 
    ORGANIZATIONAL_LEVELS.filter(validLevel => {
      const similarity = calculateStringSimilarity(normalized.toLowerCase(), validLevel.toLowerCase());
...
export const getGolonganTestCases = () => {
  return [
    ...
    { input: 'III/b', expected: { level: 'III', grade: 'b', formatted: 'III/b' } },
    ...
    { input: '3-b', expected: { level: 'III', grade: 'b', formatted: 'III/b' } },
